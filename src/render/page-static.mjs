 ...
 * Full search results.
 *
 * The homepage combobox shows eight matches, which is the right size for a
 * popover and hopeless for a broad query: "e3" matches 131 entries and "e"
 * matches 1,374, and there was no way to reach the rest. This page is where
 * the Lookup button lands when the query is not an outright identifier match.
 *
 * It is deliberately not in the sitemap and carries noindex: it is a view of
 * data that already has 1,428 indexable pages of its own, and robots.txt
 * disallows the ?q= form anyway.
 */
export function renderSearchPage({ meta, assets, counts }) {
  const body = html`<div class="prose">
      ${breadcrumb([{ href: '/', label: 'Home' }, { label: 'Search' }])}
      <h1>Search</h1>

      <form class="search search-page-form" role="search" action="/search/" method="get" id="search-form">
        ...
        <span id="q-hint" class="vh">Results update as you type.</span>
      ...
      <noscript>
        <p class="note">
          ...
          both of which filter without it.
        </p>
      </noscript>
    </div>`;
  ...
  return renderPage({
    ...
    description:
      'Search every Microsoft 365 license SKU and service plan by name, part number, GUID or product name.',
    path: '/search/',
    body,
    assets,
